...
#
# print(player.blocks)
print(background.coordinates['block'])
    ...
    if player.velocity != [0, 0]:
        for block_coordinates in background.coordinates['block']:
            if collision(combine_lists(player.coordinates, player.velocity, '+'), player.dimensions(),
                ...
                divisor = max(abs(player.velocity[0]), abs(player.velocity[1]))
                velocity = list(player.velocity)
                velocity_change = (player.velocity[0] / divisor, player.velocity[1] / divisor)
                print(velocity_change, velocity)
                while True:
                    ...
                    if result == 0:
                        ...
                        # player.conditions['falling']['old_velocity'] = 0
                        # if player.velocity[1] > 0:
                        #     player.object = object
                        #     if player.velocity[1] > 30:
                        #         player.health -= 10
