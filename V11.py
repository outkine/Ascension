...
class Background:
    def __init__(self, maps, block_sprites, block_color_values, block_offsets, block_info, timer_margins,
                 timer_gap, title_sprite, transition_height, transition_quadratic_speeds, solid_block_types, partial_solid_block_types,
                 switch_pairs):
        self.maps = maps
    ...
    def update_level(self):
        ...
        # print(self.player_default_coordinates)
                ...
                if block_type in ('laser', 'cannon', 'gate_head'):
                    if block_type in ('laser', 'gate_head'):
                        ...
                        if block_type == 'laser':
                            ...
                            block.sprite_speed = block.entity_frequency / len(block.current_sprites())
                        else:
                            block = GateHead(sprites, coordinates, 'gate_head', entity_sprite, direction, direction_index,
                ...
                if block_type == 'rail':
                    ...
                    for direction in self.directions:
                        ...
                        if (grid_coordinates in blocks and (blocks[
                            grid_coordinates] not in background.solid_block_types or blocks[grid_coordinates] in background.partial_solid_block_types)) or (
...
background_maps = background_map_sheet.get_sprites(y_constant=50, x_constant=(50, 10), scale=False)
...
#       background_switch_pairs = ({}, {}, {(40, 24): (25, 26)})
...
background = Background(background_maps, background_sprites, background_color_values, background_offsets,
                        ...
                        ('block', 'cannon', 'gate_head', 'gravity_switch', 'support_block', 'laser'), ('laser',), background_switch_pairs)
...
player.coordinates = background.convert_from_grid((37, 28))
...
while True:
    ...
    if background.condition:
        ...
        if player.block:
            ...
            if (
                ...
                # if type(previous_block) == Platform:
                #     for platform in background.platforms:
                #         if platform.direction == player.gravity:
                #             pass
                #         if (
                #                         collision(coordinates[0], dimensions, platform.coordinates,
                #                                   platform.dimensions) is True and
                #                         collision(coordinates[1], dimensions, platform.coordinates,
                #                                   platform.dimensions) is not True
                #         ):
                ...
                #             break
                #
                # elif type(previous_block) == GateHead:
                #     for gate_head in background.gate_heads:
                #         for gate in gate_head.entities:
                #             if (
                #                             collision(coordinates[0], dimensions, gate.coordinates,
                #                                       gate.dimensions) is True and
                #                             collision(coordinates[1], dimensions, gate.coordinates,
                #                                       gate.dimensions) is not True
                #             ):
                #                 player.block = gate
                #                 if gate_head.retracting:
                #                     player.velocity[gate_head.direction_index] = gate_head.speed * abs(
                #                         gate_head.direction[gate_head.direction_index])
                #                 break
                #
                # else:
                #     all_grid_coordinates = [background.find_all_grid_coordinates(coordinates[i], dimensions) for i in
                #                             range(2)]
                #     print(all_grid_coordinates)
                #     for i in range(len(all_grid_coordinates[0])):
                #         if background.block_type(all_grid_coordinates[0][i]) in background.solid_block_types and \
                #                         background.block_type(
                #                             all_grid_coordinates[1][i]) not in background.solid_block_types:
                #             player.block = background.blocks[all_grid_coordinates[0][i]]
                #             if player.block.kind == 'gravity_switch':
                #                 player.set_gravity(player.block)
                #             break
        ...
        print(player.block, player.conditions)
        ...
        for collision_type in ('platform', 'block', 'gate'):
            ...
            if collision_type == 'platform':
                for platform in background.platforms:
                    print(platform.all_grid_coordinates)
                ...
                for grid_coordinates in player.all_grid_coordinates:
                    if background.block_type(grid_coordinates) in background.solid_block_types:
                        ...
                        player.process_collision(collided_object)
                        if collided_object.kind == 'gravity_switch':
    ...
    if background.level != 0:
        ...
        for grid_coordinates in background.blocks:
            if grid_coordinates not in background.doors and grid_coordinates not in background.rails and background.blocks[grid_coordinates].kind:
                block = background.blocks[grid_coordinates]
