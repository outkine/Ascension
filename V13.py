...
class Background:
    ...
    def get_block_info(self, block_type, info):
        if self.level in self.block_info[self.difficulty] and block_type in self.block_info[self.difficulty][self.level] and info in \
        ...
        for initial_coordinates in blocks:
            ...
            if block_type in self.block_types['directional']:
                ...
                if block_type in ('laser', 'cannon', 'gate_head'):
                    ...
                    # if block_type in ('platform_tunnel_entrance', 'platform_tunnel_exit'):
                    #     # if block_type == 'platform_tunnel_exit':
                    #     block.kind = 'platform_tunnel'
                    #     # if block_type == 'platform_tunnel_entrance':
                    #     # block.rails = rails
                    #     # block.frequency = self.block_info['platform_tunnel']['frequency']
                    #     # block.platform_initial_coordinates = find_center(block.dimensions, self.sprites['platform'][0].get_size())
                    #     # block.platform_initial_coordinates[block.direction_index] -= self.block_size['platform_tunnel']['platform_offset'] * block.direction[block.direction_index]
                    #     # block.platforms = []
            ...
            if block_type in ('platform', 'platform_tunnel_entrance'):
                repeating = True
                if block_type == 'platform':
                    start = initial_coordinates
                else:
                    # start_2 = list(initial_coordinates)
                    # start_2[opposite(direction_index)] += 1
                    # start_1 = list(start_2)
                    # start_1[block.direction_index] -= 1 * block.direction[block.direction_index]
                    # rails = [start_1, start_2]
                    # current_coordinates = start_2
                    start = list(initial_coordinates)
                ...
                while True:
                    ...
                    for direction in self.directions:
                        ...
                        if color[2] == 255 and color[3] == 255:
                            directions.append(direction)
                    ...
                    if len(directions) == 1:
                        ...
                        self.exception('Single rail', initial_coordinates)

                    else:
                        self.exception('Surrounded rail', initial_coordinates)
                ...
                if block_type == 'platform':
                    self.platforms.append(
                        Platform(self.sprites['platform'], rails, self.get_block_info('platform', 'speed'), repeating))
                    continue

                else:
                    block.rails = rails
                    block.frequency = self.get_block_info('platform_tunnel', 'frequency')
                    # block.platform_initial_coordinates = self.convert_from_grid(start)
                    # block.platform_initial_coordinates[block.direction_index] -= self.block_info['platform_tunnel'][
                    #                                                                  'platform_offset'] * \
                    #                                                              block.direction[block.direction_index]
                    block.platforms = []
                    self.platform_tunnel_entrances.append(block)
            ...
            if block_type in ('entrance', 'exit'):
                if block_type == 'entrance':
...
background_maps_hard = map_sheet.get_sprites(y_constant=25, x_constant=(25, 10), scale=False)
...
block_names = (
    ...
    'gravity_switch', 'platform', 'spikes', 'alternating_block', 'platform_tunnel_entrance',
    'platform_tunnel_exit'
)
...
default_background_block_info = {
    'platform': {'speed': 1},
    'cannon': {'entity_speed': 2, 'entity_frequency': 2 * game.speed},
    'laser': {'inactive_duration': 2 * game.speed, 'active_duration': 1 * game.speed},
    'gate_head': {'speed': 1},
    'alternating_block': {'frequency': 2 * game.speed},
    'platform_tunnel': {'frequency': 3 * game.speed, 'platform_offset': 3}
}

background_block_info = {
    'easy': {
        8: {'laser': {'inactive_duration': 40}},
        9: {'platform_tunnel': {'frequency': 1 * game.speed}}
    },
    'hard': {8: {'laser': {'inactive_duration': 40, 'active_duration': 40}}}
}
...
#     player.coordinates = background.blocks[block].coordinates
        ...
        for platform_tunnel_entrance in background.platform_tunnel_entrances:
            ...
            if (game.count - platform_tunnel_entrance.delay) % platform_tunnel_entrance.frequency == 0:
                platform = Platform(background.sprites['platform'], platform_tunnel_entrance.rails,
                                    background.get_block_info('platform', 'speed'), False)
    ...
    if background.level != 0:
        ...
        for door in background.doors:
            game.display.blit(background.sprites['door_background'][0],
                              player.generate_display_coordinates(door.background_coordinates))
