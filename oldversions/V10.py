            ...
            if block_type in ('laser', 'cannon', 'gate_head'):
                ...
                if block_type in ('laser', 'gate_head'):
                    ...
                    if block_type == 'laser':
                        ...
                        block = EntityBlock(sprites, coordinates, 'gate_head', entity_sprite, direction)
                ...
                if block_type == 'gate_head':
                    ...
                    block.entities[tuple(active_coordinates)] = Thing(block.entity_sprite, self.add_offset(
                ...
                while True:
                    ...
                    if block_type in ('laser', 'gate_head'):
                        ...
                        entity = Thing(block.entity_sprite, self.add_offset(
                            ...
                            block.direction))
                        if block_type == 'laser':
                            entity.grid_coordinates = tuple(active_coordinates)
                            block.entities.append(entity)
                        else:
                            # noinspection PyTypeChecker
                            block.entities[tuple(active_coordinates)] = entity
...
background_block_names = ('block', 'lava', 'fire', 'flag', 'cannon', 'entrance', 'exit', 'laser', 'rail', 'gate_head', 'switch')
...
while True:
    ...
    if background.condition:
        ...
        for collision_type in ('platform', 'gate', 'blocks'):
            player.update_grid_coordinates()
            ...
            elif collision_type == 'gate':
                for grid_coordinates in player.all_grid_coordinates:
                    for gate_head in background.gate_heads:
                        if grid_coordinates in gate_head.entities:
                            if collision(combine_lists(player.coordinates, player.velocity, '+'), player.dimensions,
                                         gate_head.entities[grid_coordinates].coordinates,
                                         gate_head.entities[grid_coordinates].dimensions) is True:
                                collided_object = gate_head.entities[grid_coordinates]
                                player.process_collision(collided_object)
        ...
        for laser in background.lasers:
            ...
            if laser.active:
                ...
                for entity in laser.entities:
                    if not background.condition:
                        if entity.grid_coordinates in player.all_grid_coordinates:
                            if collision(entity.coordinates, entity.dimensions,
                                         player.coordinates,
                                         player.dimensions) is True:
                                player.die()
                    ...
                    for gate_head in background.gate_heads:
                        if entity.grid_coordinates in gate_head.entities:
                            if collision(entity.coordinates, entity.dimensions,
                                         gate_head.entities[entity.grid_coordinates].coordinates,
                                         gate_head.entities[entity.grid_coordinates].dimensions) is True:
                                laser.end = (entity, gate_head.entities[entity.grid_coordinates])
                                break

                    for platform in background.platforms:
                        if entity.grid_coordinates in platform.all_grid_coordinates:
                            if collision(entity.coordinates, entity.dimensions,
    ...
    if background.level != 0:
        ...
        for gate_head in background.gate_heads:
            for entity in gate_head.entities:
                game.blit(gate_head.entities[entity])
