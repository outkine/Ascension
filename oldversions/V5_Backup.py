...
class Background:
            ...
            for y in range(self.maps[self.level - 1].get_height()):
                ...
                if color == (0, 0, 0, 0):
                    ...
                    if block_type == 'cannon':
                        if self.get_color((x + 1, y)) in self.block_color_values:
                            if self.block_color_values[self.get_color((x + 1, y))] == 'block':
                                direction = -1
                                self.block_sprites[block_type]['cannon'] = [
                                    pygame.transform.flip(sprite, True, False) for sprite in
                                    self.block_sprites[block_type]['cannon']

                                elif self.get_color((x + 1, y)) in self.block_color_values:
                                if self.block_color_values[self.get_color((x - 1, y))] == 'block':
                                    direction = 1

                                else:
                                raise Exception("Unsupported cannon at {0}".format((x, y)))

                        thing = pygame.transform.flip(self.block_sprites[block_type], (x, y), direction)

                    else:
                        thing = Thing(self.block_sprites[block_type], coordinates=(x, y))
                    self.blocks.setdefault(self.level, {}).setdefault(block_type, []).append(thing)
                else:
            ...
            for block_type in self.blocks[self.level]:
                for block in self.blocks[self.level][block_type]:
                    ...
                    if block_type in self.block_offsets:
                        block.coordinates = combine_lists(block.coordinates,
    ...
    class Cannon(Thing):
        def __init__(self, sprites, coordinates, direction):
            ...
            self.shot_dimensions = self.sprites['shot'].get_size()
            if self.direction == 1:
    ...
    background_block_names = ['block', 'lava', 'fire', 'flag']
    ...
    background_sprites['cannon'] = {
        ...
        'cannon': background_objects_raw[4]
    }
    ...
    player_sprites_raw = sprite_sheet.get_sprites(y_constant=10, x_constant=(10, 7))
