...
class Quadratic:
    ...
    def execute(self):
        ...
        print(self.get_y(self.current_x), self.current_x, self.x_range[1])
...
background_objects_raw = sprite_sheet.get_sprites(y_constant=10, x_constant=(10, 4))

background_sprites = {
    'block': background_objects_raw[0],
    'flag': background_objects_raw[1],
    'entrance': background_objects_raw[2],
    'fire': background_objects_raw[3]
}
...
while True:
    ...
    if (keys[K_UP] or keys[K_w]) and player.on_block is True and not player.conditions['jumping']['active']:
        player.conditions['jumping']['active'] = True
        # noinspection PyTypeChecker
        player.conditions['jumping']['quadratic'] = Quadratic(1, (0, player.conditions['jumping']['velocity']),
                                                              player.conditions['jumping']['speed'] * game.speed)
    ...
    if not player.on_block and not player.conditions['falling']['active'] and not player.conditions['jumping'][
        ...
        # noinspection PyTypeChecker
        player.conditions['falling']['quadratic'] = Quadratic(1, (0, player.conditions['falling']['velocity']),
                                                              player.conditions['falling']['speed'] * game.speed)
