...
background_sprites['laser'] = sprite_sheet.get_sprites(y_constant=8, x_constant=(10, 4))
...
background_maps = background_map_sheet.get_sprites(y_constant=50, x_constant=(50, 3), scale=False)
...
background_level_times = ((600, 10), (60, 5), (120, 20))
...
background_switch_pairs = ({(27, 8): (27, 19)},)
...
background = Background(background_maps, background_sprites, background_color_values, background_offsets,
                        ...
                        ('block', 'cannon', 'gate_head'), background_switch_pairs)

# background.level = 2
background.condition = None
background.update_level()
player.coordinates = background.convert_from_grid((45, 34))
...
while True:
    ...
    if background.condition:
        ...
        all_grid_coordinates_1 = background.find_all_grid_coordinates(
            (player.coordinates[0], player.coordinates[1] + player.dimensions[1]),
            (player.dimensions[0], 1))
        all_grid_coordinates_2 = background.find_all_grid_coordinates(
            (player.coordinates[0], player.coordinates[1] + player.dimensions[1] - 1),
            (player.dimensions[0], 1))
        for i in range(len(all_grid_coordinates_1)):
            if background.block_type(all_grid_coordinates_1[i]) in background.solid_block_types and \
                            background.block_type(all_grid_coordinates_2[i]) not in background.solid_block_types:
                player.block = background.blocks[all_grid_coordinates_1[i]]
                break
            ...
            for gate_head in background.gate_heads:
                for gate in gate_head.entities:
                    if collision(
                            (player.coordinates[0], player.coordinates[1] + 1),
                            player.dimensions,
                            gate.coordinates, gate.dimensions
                    ) is True or (previous_block == gate and collision(
                        (player.coordinates[0], player.coordinates[1] + 1 + gate_head.speed),
                        player.dimensions,
                        gate.coordinates, gate.dimensions)
                                  ) is True:
                        player.block = gate
                        if gate_head.retracting:
                            player.velocity[gate_head.direction_index] = gate_head.speed * abs(gate_head.direction[
                                                                                                   gate_head.direction_index])
                        break
        ...
        if not background.condition:
            for grid_coordinates in player.all_grid_coordinates:
                ...
                if block_type:
                    ...
                    elif block_type == 'switch' and block.gate_head.sprite_index == 0:
